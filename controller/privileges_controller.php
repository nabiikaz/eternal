  ...
  if(isset($_COOKIE["LoginSession"])){
     ...
     // echo $query;
      ...
      if( $result = $sql->query($query)){
             $row = $result->fetch_assoc();
             if($row["count"] > 0){
                 return  ' <b class="my_account" > Mon compte</b>
                        <ul class="list-group my_account_list text-center collapse" style="position: absolute;z-index: 3;background-color: white;">
                            
                            <li onclick="location.href=\'settings.php\'" hidden>Settings <span  class="glyphicon glyphicon-cog"></span></li>
                            <hr>
                            <li onclick="location.href=\'authentification.php?action=logout\'">Se déconnecter<span style="margin-left:15px;" class="glyphicon glyphicon-log-out"></span></li>
